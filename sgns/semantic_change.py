...
def measure(model1, model2, change_file, mode="change"):
    ...
    if mode == "similarity":             # mb
        if wv1.index_to_key != wv2.index_to_key:                                   #mb. 
        ...
        change = dict(zip(common_vocab, cos_sim(vecs1, vecs2)))                    #mb
...
if __name__ == '__main__':
    ...
    parser.add_argument("change_file", type=str, help="change file")
    parser.add_argument("--similarity", "-s", action="store_true", help="provide to use cosine similarity as measure (default: cosine change)")

    args = parser.parse_args()

    mode = "similarity" if args.similarity else "change"

    measure(Path(args.model1), Path(args.model2), Path(args.change_file), mode)
